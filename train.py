...
def visualize_cycle(opt, real_A, visuals, eidx, uidx, train):
    ...
    images = [img.cpu() for img in list(visuals.values())]
...
def train_model():
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(train_dataset):
            real_A, real_B = Variable(data['A']).type(dtype), Variable(data['B']).type(dtype)
        ...
        with torch.no_grad():
            if epoch % opt.eval_A_freq == 0:
                ...
                if float(dev_mse_A.cpu()) < results['best_dev_mse_A']:
                    with open("%s/best_mse_A.txt" % opt.expr_dir, 'w') as best_mse_A_f:
                        best_mse_A_f.write(res_str_list[0]+'\n')
                        best_mse_A_f.flush()
                    results['best_dev_mse_A'] = float(dev_mse_A.cpu())
                    results['best_test_mse_A'] = float(test_mse_A.cpu())
                    model.save('best_A')
                    save_results(opt.expr_dir, results)
            ...
            if epoch % opt.eval_B_freq == 0:
                ...
                if float(dev_bpp_B.cpu()) < results['best_dev_bpp_B']:
                    with open("%s/best_bpp_B.txt" % opt.expr_dir, 'w') as best_bpp_B_f:
                        best_bpp_B_f.write(res_str_list[0]+'\n')
                        best_bpp_B_f.flush()
                    results['best_dev_bpp_B'] = float(dev_bpp_B.cpu())
                    results['best_test_bpp_B'] = float(test_bpp_B.cpu())
                    save_results(opt.expr_dir, results)
                    model.save('best_B')
