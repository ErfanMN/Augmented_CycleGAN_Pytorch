...
def eval_mse_A(dataset, model, use_gpu=True):
    ...
    return torch.mean(torch.stack(mse_A))
...
def eval_ubo_B(dataset, model, steps=500, visualize=False, vis_name=None, vis_path=None,
    ...
    for batch in dataset:
        ...
        bpp_B.append(bpp)
        kld_B.append(kld)
    return torch.mean(torch.stack(ubo_B)), torch.mean(torch.stack(bpp_B)), torch.mean(torch.stack(kld_B))
...
def variational_ubo(model, real_A, real_B, steps, visualize=False, vis_name=None, vis_path=None,
    ...
    for i in range(steps):
        ...
        ubo = (-log_prob + kld) + (128*128*3) * math.log(127.5)
        ubo_val_new = ubo.mean(0).data
        kld_val = kld.mean(0).data
        bpp = ubo.mean(0).data / (128*128*3* math.log(2.))
        if compute_l1:
        ...
        ubo_val = ubo_val_new
        loss = Variable(ubo.mean(0) ,requires_grad = True)
...
def visualize_data(opt, data, size, save_path):
    images = [img.cpu() for img in data]
